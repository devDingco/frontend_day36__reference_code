...
export const useBoardWrite = (isEdit: boolean, fileInputRefs) => {
  ...
  const [hoveredIndex, setHoveredIndex] = useState(null); // 어떤 이미지에 hover했는지 추적
  ...
  const onChangeFile = async (event, index) => {
    ...
    console.log("inChangeFIle 에서 index", index);
